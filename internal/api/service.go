...
func (s *Service) Query(ctx context.Context, p memory.Principal, req memory.QueryRequest) (memory.EvidencePack, error) {
	...
	if mode == memory.ModeBoot {
		...
		s.recordAction(ctx, p, req, queryID, memory.ActionSearch, started, true)
		return ep, nil
	}
	...
	if mode == memory.ModeWorkspace {
		...
		s.recordAction(ctx, p, req, queryID, memory.ActionWorkspaceMount, started, true)
	} else {
		s.recordAction(ctx, p, req, queryID, memory.ActionSearch, started, true)
	}
	return ep, nil
...
// Profile serves L0/L1 with a content ETag (spec §13.1).
func (s *Service) Profile(ctx context.Context, p memory.Principal, namespaceID, repo string, level int) (body string, etag string, err error) {
	if level == 0 {
		...
		ex := s.Assembler
		pk, aerr := ex.Assemble(ctx, pack.Input{
...
func (s *Service) recordAction(ctx context.Context, p memory.Principal, req memory.QueryRequest, queryID, actionType string, started time.Time, success bool) {
	ns := ""
	if len(req.NamespaceHints) > 0 {
		ns = req.NamespaceHints[0]
	}
	...
	_ = s.Store.InsertAction(ctx, memory.MemoryAction{
		...
		Output: map[string]any{}, Success: &ok, LatencyMs: int(time.Since(started).Milliseconds()),
	})
}
